SQL Maestro is a tool designed to help relieve some of the stress of working a ticket. 

Having a helper like Maestro gives you a "workspace" for organizing the important or relevant IDs, resource names, times/dates, screenshots, db tables, KB or ZenDesk links, smoke-tested workflows for challenging cases that require multiple steps/checks etc. etc.
...
![[CleanShot 2025-09-19 at 13.29.42.png]]
...
Make sure to save (cmd + s) or 'Save' button if you make any changes to the template. 

#### Query Template Change History

Each time you make a change to an existing Query Template, **that change is recording in the 'backups' folder in the Application Support for SQL Maestro**:
...
These files can pile up quickly if you make a lot of changes, so feel free to "prune" the garbage files that are un-needed. 

#### Query Backups

Navigate to top menu item "Queries" > "Backup Queries"

This will create a zip file of all queries you have in your app. Useful for version updates that could potentially (hopefully not) delete those queries.
...
This is useful because each ticket will require those fields regardless of Query Template (the scope of the ticket in question/troubleshooting case )

### **Alternate Fields**

✅ These are fields that remain available per ticket session that hold important values but may not be useful at the moment. 

- You might be working a ticket that requires you to remember multiple sig-ids, but only one is useful at a time
...
✅ Lets you copy just one field’s value to clipboard.
...
| Cmd + R     | Reload Queries (used to manually reload the Query Templates if changes were made, however, this actions is done automatically after a saving a query template if done from the in-app editor) |
